        ...
        <div class="invoice-container" style="margin-left: 75px; margin-right: 75px; margin-top: 25px; margin-bottom: 25px; text-align: initial;">
            ...
            <table style="width: 100%">
                <tr>
                    ...
                    <th style="padding-right: 5px;font-size: 16px; font-family: Arial, sans-serif;">Cantidad</th>
                ...
                <tr v-for="sale in sales" :key="sale.id">
                    <td style="padding-right: 5px;font-size: 16px; font-family: Arial, sans-serif;">{{ sale.productoCod }}</td>
                    <td style="padding-right: 5px;font-size: 16px; font-family: Arial, sans-serif;">{{ sale.productoCantidad }}</td>
                    <td style="padding-right: 5px;font-size: 16px; font-family: Arial, sans-serif;">{{ sale.productoPrecio }}</td>
                    <td style="padding-right: 5px;font-size: 16px; font-family: Arial, sans-serif;">{{ sale.facturaProductoItbis }}</td>
                    <td style="padding-right: 5px;font-size: 16px; font-family: Arial, sans-serif;">{{ sale.facturaProductoTotal }}</td>
                </tr>
            </table>
            <div class="separator" style="border-top: 1px solid black; margin-top: 18px; margin-bottom: 20px;"></div>
            <table style="width: 100%">                
...
<script>
    ...
    export default {
        ...
        created() {
            const facturaCod = this.$route.params.facturaCod;
            this.retrieveData(facturaCod)
        },
        methods: {
            retrieveData(facturaCod) {                                
                ipcRenderer.send('retrieve-bill-and-sales', facturaCod) 
            ...
            async generatePDF() {
                ...
                let width = pageWidth * 0.9; // 90% of page width
                let height = width / ratio;

                // Check if the height exceeds 90% of the page height
                ...
                pdf.addImage(imgData, 'PNG', marginX, 0, width, height);
                pdf.save(`factura-${this.bill.facturaCod}.pdf`);
            },
        },
        beforeUnmount() {
            ipcRenderer.removeAllListeners('bill-and-sales-retrieved')
        },
        setup() {
