    ...
    export default {
        ...
        methods: {
            submitForm() {
                ...
                ipcRenderer.send('add-client', clientData);

                ipcRenderer.once('client-added', (event, client) => {
                    console.log('client-added event received:', client);
                    this.isSubmitting = false;
                    this.showModal = true;
                    this.modalMessage = 'Cliente agregado exitosamente...';
                });

                ipcRenderer.once('client-already-exists', (event, client) => {
                    console.log('client-already-exists event received:', client);
                    this.isSubmitting = false;
                    this.showModal = true;
                    this.modalMessage = 'Ya existe un cliente con ese DNI...';
                });

                ipcRenderer.once('error-adding-client', (event, error) => {
                    console.log('error-adding-client event received:', error);
                    this.isSubmitting = false;
                    this.showModal = true;
                    this.modalMessage = 'Ha ocurrido un error al intentar agregar el cliente...';
        ...
        beforeUnmount() {
            ...
            ipcRenderer.removeAllListeners('client-already-exists');
        },
        setup() {
