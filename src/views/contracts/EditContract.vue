<template>
    ...
    <div class="card shadow-sm rounded">
        <div class="card-body text-left">
            <h1 class="card-title view-title">Modificar contrato</h1>
            ...
            async retrieveServices() {                
                ...
                ipcRenderer.once('services-retrieved', (event, services) => {
                    ...
                    ipcRenderer.once('contract-retrieved', (event, contract) => {
                        this.contract = {
                            ...contract,
                            contratoFechaInicio: new Date(contract.contratoFechaInicio).toISOString().split('T')[0],
                            contratoFechaVencimiento: new Date(contract.contratoFechaVencimiento).toISOString().split('T')[0]
                        }
                        this.selectedService = this.services.find(service => service.servicioCod === this.contract.servicioCod)
                        this.totalPrice = this.contract.servicioPrecio
                    })
                })
            },    
            submitForm() {
                ...
                ipcRenderer.send('update-contract', contractData);

                ipcRenderer.once('contract-updated', (event, contract) => {
                    console.log('contract-updated event received:', contract);
                    this.isSubmitting = false;
                    this.showModal = true;
                    this.modalMessage = 'Contrato modificado exitosamente...';
                });
                
                ipcRenderer.once('error-updating-contract', (event, error) => {
                    console.log('error-updating-contract event received:', error);
                    this.isSubmitting = false;
                    this.showModal = true;
                    this.modalMessage = 'Ha ocurrido un error al intentar modificar el contrato...';
